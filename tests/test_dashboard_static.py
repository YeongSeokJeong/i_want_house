...
class DashboardStaticTests(unittest.TestCase):
    ...
    def test_dashboard_exposes_collection_history_and_clear_trend_copy(self) -> None:
        ...
        self.assertIn("데이터 품질/기준선 준비 상태", html)
        self.assertIn("dataReadinessBody", html)
        self.assertIn("0건 연속", html)
        self.assertIn("품질 차단", html)
    ...
    def test_dashboard_fetches_committed_json_state(self) -> None:
        ...
        self.assertIn("renderDataReadiness", script)
        self.assertIn("consecutiveZeroListings", script)
        self.assertIn("qualityBlockStatus", script)
        self.assertIn("readinessSourceStatus", script)
    ...
    def test_dashboard_has_section_local_error_and_empty_states(self) -> None:
        ...
        self.assertIn("데이터 준비 상태를 불러오지 못했습니다.", script)
        self.assertIn("실거래 기준선", script)
        self.assertIn("수집 정상", script)
        self.assertIn("0건 확인", script)
        self.assertIn("후보 없음", script)
