...
class CandidateAnalyzer:
    ...
    def apply_quality_blocks(self, candidates: list[Candidate], blocks: dict[str, QualityBlock]) -> list[Candidate]:
        if not blocks:
            return candidates
    ...
    def approved(self, candidates: list[Candidate], limit: int = 5) -> list[Candidate]:
...
def classify_candidates(
    ...
    return CandidateAnalyzer().classify(targets, records_by_complex, notified_state, trade_baselines)
...
def detect_average_price_jumps(
    ...
    *,
    ...
    return CandidateAnalyzer().detect_average_price_jumps(
        records_by_complex,
        previous_averages,
        threshold=threshold,
    )
...
def apply_quality_blocks(candidates: list[Candidate], blocks: dict[str, QualityBlock]) -> list[Candidate]:
    return CandidateAnalyzer().apply_quality_blocks(candidates, blocks)


def approved_candidates(candidates: list[Candidate], limit: int = 5) -> list[Candidate]:
    return CandidateAnalyzer().approved(candidates, limit)
