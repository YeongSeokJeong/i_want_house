...
## Telegram update 공통 처리
- `src/jeonseloop/telegram_updates.py`는 Telegram update payload 추출, `update_id` 파싱, chat 필터링, 상태 병합, `.env` 로딩, 텍스트 redaction, `getUpdates` 호출 helper를 제공한다.
- backlog intake와 ops proposal은 이 공통 모듈을 사용하되, 백로그 행 생성과 운영 proposal 생성 판단은 각 모듈 안에 남긴다.
- 공통 처리 계층은 Telegram 메시지를 보내지 않으며, 실제 알림 전송 조건은 제품 루프의 `--send` 게이트와 별도로 유지한다.
...
## 아키텍처 메모
`src/jeonseloop/`의 제품 루프는 `LoopCoordinator`가 한 사이클을 조율하고, 수집(`ListingCollector`), 검증(`ListingValidator`), 후보 판정(`CandidateAnalyzer`), 진단 생성(`LoopDiagnostics`), 상태 저장(`LoopStateRepository`), 거래 기준선(`TradeBaselineRepository`), 알림(`NotificationService`), LLM 검수(`CandidateReviewService`), 기준 제안(`CriteriaSuggestionService`) 객체를 명시적으로 조합하는 구조를 기준으로 한다. `LoopStateRepository`는 호환 facade로 유지하고, `state_repositories.py`가 listing snapshot, history, notified state, health, collector diagnostics, urgent feed, criteria log 저장 책임을 대상별 repository로 나눈다. 모든 JSON 상태 쓰기는 `JsonStateStore.atomic_write_json`을 공유해 validate-before-replace 규칙을 유지한다. `models.py`는 `NormalizedListing`, `CandidateRecord`, `RunRecord`, `FeedItem` 변환 계층을 제공해 listing/candidate/run/feed의 핵심 필드를 런타임에서 검증하고, 기존 JSON 출력 호환을 위해 알 수 없는 provider/dashboard 필드는 보존한다. Telegram update 처리에서는 `telegram_updates.py`가 공통 payload/parser/helper 계층이고, backlog intake와 ops intake는 각자의 도메인 판단만 담당한다. 기존 module-level 함수들은 CLI와 테스트 호환을 위한 얇은 wrapper로 유지한다.
