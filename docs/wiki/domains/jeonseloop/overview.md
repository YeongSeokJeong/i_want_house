...
- 대시보드는 정적 파일만 사용하며 `data/state/health.json`, `data/history/*.json`, `data/state/urgent-feed.json`을 fetch한다. 첫 화면의 `단지별 감시 상태`는 watchlist 기준과 단지별 최신 history를 조합해 희망가, 최근 최저 호가, 실거래 기준선, 급매선, 괴리율, 상태를 보여주고, 최근 급매 후보는 보조 영역으로 표시한다. `health.json`의 `runs`는 최근 수집/검색 실행 이력으로 표시하고, 단지별 가격 추이는 수집된 매매 매물의 최저/평균 호가 이력으로 해석한다.
